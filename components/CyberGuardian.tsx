        ...
        <>
            ...
            <button
                ...
                className={`fixed bottom-24 right-8 z-[60] flex items-center justify-center transition-all duration-300 shadow-[0_0_20px_rgba(0,255,100,0.3)] hover:shadow-[0_0_30px_rgba(0,255,100,0.5)] ${isOpen
                    ? 'w-12 h-12 rounded-full bg-rose-500 hover:bg-rose-600 rotate-90'
                    : 'w-14 h-14 rounded-2xl bg-white dark:bg-[#0a0a0a] border border-emerald-500/50 hover:scale-105'}`}
            >
                {isOpen ? (
            ...
            <div className={`fixed bottom-24 right-4 md:right-8 z-[60] w-[90vw] md:w-[400px] bg-white/95 dark:bg-[#0a0a0a]/95 backdrop-blur-xl border border-emerald-500/30 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 origin-bottom-right flex flex-col ${isOpen ? 'opacity-100 scale-100 translate-y-0 h-[600px] max-h-[80vh]' : 'opacity-0 scale-90 translate-y-10 h-0 pointer-events-none'}`}>
                ...
                {/* Header */}
                <div className="bg-gradient-to-r from-emerald-50 to-white dark:from-emerald-900/50 dark:to-black p-4 border-b border-gray-100 dark:border-white/10 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                        <Bot className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
                    </div>
                    <div>
                        <h3 className="font-bold text-gray-900 dark:text-white text-sm uppercase tracking-wider">Cyber Guardian</h3>
                    ...
                    <button
                        ...
                        className="ml-auto p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg transition-colors"
                    >
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 font-sans">
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center ${msg.role === 'model' ? 'bg-emerald-100 dark:bg-emerald-500/10' : 'bg-gray-200 dark:bg-gray-700'}`}>
                                {msg.role === 'model' ? <Shield className="w-4 h-4 text-emerald-600 dark:text-emerald-500" /> : <MessageSquare className="w-4 h-4 text-gray-700 dark:text-white" />}
                            </div>
                            <div className={`p-3 rounded-2xl text-sm leading-relaxed max-w-[85%] ${msg.role === 'model'
                                ? 'bg-gray-100 dark:bg-[#151515] border border-gray-200 dark:border-emerald-500/20 text-gray-800 dark:text-gray-200'
                    ...
                    {isLoading && (
                        <div className="flex gap-3">
                            ...
                            <div className="bg-gray-100 dark:bg-[#151515] border border-emerald-200 dark:border-emerald-500/20 px-4 py-2 rounded-2xl">
                ...
                {/* Input Area */}
                <div className="p-4 bg-gray-50/80 dark:bg-black/50 border-t border-gray-200 dark:border-white/5">
                    <form onSubmit={sendMessage} className="relative">
                        <input
                            ...
                            className="w-full bg-white dark:bg-[#111] border border-gray-200 dark:border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm text-gray-900 dark:text-white focus:border-emerald-500/50 outline-none transition-all placeholder:text-gray-500 dark:placeholder:text-gray-600 disabled:opacity-50"
                        />
                        <button
                            type="submit"
