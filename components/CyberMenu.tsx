...
const CyberMenu: React.FC<Props> = ({ onNavigate, onBack, onSelectPresentation }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 md:py-20 relative z-10">
      {/* Sticky Navigation */}
      <div className={`fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-gray-200 dark:border-white/10 transition-all duration-300 transform ${isSticky ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'}`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center border border-amber-200 dark:border-amber-500/20 group-hover:bg-amber-100 dark:group-hover:bg-amber-500/20 transition-colors">
              <Newspaper className="w-5 h-5 text-amber-600 dark:text-amber-500" />
            </div>
            <span className="font-display font-bold text-gray-900 dark:text-white uppercase tracking-tight hidden md:block">CyberSec <span className="text-amber-600 dark:text-amber-500">Hub</span></span>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4 overflow-x-auto no-scrollbar mask-grad-x flex-1 justify-center px-4">
            {sections.map(item => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all hover:bg-gray-100 dark:hover:bg-white/10 flex items-center gap-2 whitespace-nowrap ${activeSection === item.id ? `bg-${item.color}-50 dark:bg-${item.color}-500/10 text-${item.color}-600 dark:text-${item.color}-400 border border-${item.color}-200 dark:border-${item.color}-500/20` : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white border border-transparent'
                  }`}
              >
          ...
          <button onClick={onBack} className="w-10 h-10 rounded-full bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 flex items-center justify-center transition-all border border-gray-200 dark:border-white/5 group" title="Zpět">
            <ArrowLeft className="w-4 h-4 text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white transition-colors" />
          </button>
        </div>
      </div>
      ...
      >
        <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 flex items-center justify-center group-hover:border-cyan-500/30 transition-all">
      ...
      <header className="mb-16 animate-fade-in-up text-center md:text-left">
        <h1 className="text-4xl md:text-6xl font-display font-bold text-gray-900 dark:text-white mb-6 uppercase tracking-tight">
          KYBER <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-cyan-500 dark:from-emerald-400 dark:to-cyan-400">BEZPEČNOST</span>
        </h1>
        <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl leading-relaxed mb-6">
        ...
        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl max-w-3xl inline-block text-left">
          <p className="text-emerald-700 dark:text-emerald-400 flex items-start gap-3 text-sm md:text-base">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in-up mb-32" style={{ animationDelay: '200ms' }}>
        {sections.map((item) => (
          ...
          >
            ...
            <div className="relative h-full bg-white/60 dark:bg-[#0a0a0a]/60 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl p-8 overflow-hidden flex flex-col min-h-[220px]">
              <div className="flex items-start justify-between mb-6">
                <div className={`bg-${item.color}-50 dark:bg-${item.color}-500/10 p-4 rounded-2xl border border-${item.color}-200 dark:border-${item.color}-500/20 group-hover:scale-110 transition-transform`}>
                  <item.icon className={`w-6 h-6 text-${item.color}-600 dark:text-${item.color}-400`} />
                </div>
                <ArrowRight className="w-5 h-5 text-gray-500 dark:text-gray-700 group-hover:text-gray-900 dark:group-hover:text-white transition-colors rotate-90 md:rotate-0" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2 font-display uppercase tracking-tight">{item.title}</h3>
        ...
        {sections.map((item) => (
          <div key={item.id} ref={item.ref} className="scroll-mt-24 min-h-[50vh] animate-on-scroll">
            <div className="flex items-center gap-4 mb-8 border-b border-gray-200 dark:border-white/10 pb-4">
              <div className={`p-3 rounded-xl bg-${item.color}-50 dark:bg-${item.color}-500/10 border border-${item.color}-200 dark:border-${item.color}-500/20`}>
                <item.icon className={`w-6 h-6 text-${item.color}-600 dark:text-${item.color}-400`} />
              </div>
              <h2 className="text-3xl md:text-4xl font-display text-gray-900 dark:text-white uppercase tracking-tight">{item.title}</h2>
            </div>
            <p className="text-gray-600 dark:text-gray-400 text-lg mb-8 max-w-4xl leading-relaxed font-light border-l-2 border-gray-200 dark:border-white/10 pl-6">
              {item.desc}
            </p>

            <div className="bg-white/40 dark:bg-[#0a0a0a]/40 border border-gray-200 dark:border-white/5 rounded-3xl p-1 md:p-6 shadow-2xl">
      ...
      <button
        ...
        className="fixed bottom-8 right-8 bg-white/50 dark:bg-white/10 hover:bg-white/80 dark:hover:bg-white/20 text-gray-900 dark:text-white p-4 rounded-full backdrop-blur-md border border-gray-200 dark:border-white/10 shadow-2xl z-50 transition-all opacity-50 hover:opacity-100"
        title="Nahoru"
      >
